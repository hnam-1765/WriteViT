...
def G_arch(ch=64, attention="64", ksize="333333", dilation="111111"):
    ...
    arch[31] = {
        "in_channels": [ch * item for item in [16, 16, 4, 2]],
    ...
    arch[16] = {
        "in_channels": [ch * item for item in [8, 4, 2]],
        ...
        **kwargs
        ...
        if self.first_layer:
            if self.one_hot:
                self.linear = self.which_linear(
                    self.dim_z // self.num_slots + self.n_classes,
                    self.arch["in_channels"][0]
                    * (self.bottom_width * self.bottom_height),
                )
            else:
                self.linear = self.which_linear(
                    self.dim_z // self.num_slots + 1,
                    self.arch["in_channels"][0]
                    * (self.bottom_width * self.bottom_height),
                )
            if self.one_hot_k == 1:
            ...
            if self.one_hot_k > 1:
                self.linear = self.which_linear(
                    self.dim_z // self.num_slots + self.n_classes * self.one_hot_k,
                    self.arch["in_channels"][0]
                    * (self.bottom_width * self.bottom_height),
                )
        ...
        for index in range(len(self.arch["out_channels"])):
            if "kernel1" in self.arch.keys():
                ...
                conv1 = functools.partial(
                    layers.SNConv2d,
                    kernel_size=self.arch["kernel1"][index],
                    padding=padd1,
                    num_svs=num_G_SVs,
                    num_itrs=num_G_SV_itrs,
                    eps=self.SN_eps,
                ...
                conv2 = functools.partial(
                    layers.SNConv2d,
                    kernel_size=self.arch["kernel2"][index],
                    padding=padd2,
                    num_svs=num_G_SVs,
                    num_itrs=num_G_SV_itrs,
                    eps=self.SN_eps,
                    ...
                    [
                        layers.GBlock(
                            ...
                            upsample=(
                                ...
                                if index < len(self.arch["upsample"])
                                else None
                            ),
                        )
                    ]
...
def D_arch(ch=64, attention="64", input_nc=3, ksize="333333", dilation="111111"):
    arch = {}
    arch[256] = {
        "in_channels": [input_nc] + [ch * item for item in [1, 2, 4, 8, 8, 16]],
    ...
    arch[128] = {
        "in_channels": [input_nc] + [ch * item for item in [1, 2, 4, 8, 16]],
    ...
    arch[32] = {
        "in_channels": [input_nc] + [item * ch for item in [4, 4, 4]],
    ...
    arch[16] = {
        "in_channels": [input_nc] + [ch * item for item in [1, 4, 8]],
    ...
    arch[17] = {
        "in_channels": [input_nc] + [ch * item for item in [1, 4]],
    ...
    arch[20] = {
        "in_channels": [input_nc] + [ch * item for item in [1, 8, 16]],
